...
static esp_err_t rest_command_post_handler(httpd_req_t *req);
...
esp_err_t start_rest_server(void)
{
    ...
    httpd_uri_t command_post_uri = {
        .uri = "/api/command",
        .method = HTTP_POST,
        .handler = rest_command_post_handler,
...
/**
 * @brief Handler para atender los request de los clientes que envien un comando.
 *
 * @param req mensaje de request del cliente.
 * @return esp_err_t.
 */
static esp_err_t rest_command_post_handler(httpd_req_t *req)
{
    ...
    {
        ESP_ERROR_CHECK(httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "content too long"));
        return ESP_FAIL;
    }
    int cur_len = 0;
    ...
    while (cur_len < total_len)
    {
        ...
        if (received <= 0)
        {
            ESP_ERROR_CHECK(httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Failed to post control value"));
            return ESP_FAIL;
        }
        cur_len += received;
    }
    ...
    int command = cJSON_GetObjectItem(root, "command")->valueint;
    if (channel == 1)
    {
        write_led_command(&command);
        ESP_LOGI(TAG, "Channel 1 command = %d", command);
        cJSON_Delete(root);
        ESP_ERROR_CHECK(httpd_resp_sendstr(req, "Post control value successfully"));
        return ESP_OK;
    }
    cJSON_Delete(root);
    ESP_ERROR_CHECK(httpd_resp_send_err(req, HTTPD_500_INTERNAL_SERVER_ERROR, "Post control value invalid"));
    return ESP_FAIL;
}